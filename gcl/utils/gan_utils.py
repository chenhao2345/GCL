...
def get_data_loader_list(root, file_list, batch_size, train, new_size=None,
    ...
    transform_list = [transforms.Resize((height, width),
                                        interpolation=3)] + transform_list if new_size is not None else transform_list
...
def display_images(imgs, imgs_mesh_nv, imgs_out, split, train_writer, iterations):
    imgs = denormalize(imgs)
    meshes_nv = denormalize_mesh(imgs_mesh_nv)
    # meshes_nv = imgs_mesh_nv

    imgs_recon = denormalize_recon(imgs_out[0])
    imgs_nv = denormalize_recon(imgs_out[1])
    imgs_nv2recon = denormalize_recon(imgs_out[2])

    train_writer.add_images(tag='rgb_' + split, global_step=iterations, img_tensor=imgs)
    train_writer.add_images(tag='mesh_' + split, global_step=iterations, img_tensor=meshes_nv)
    train_writer.add_images(tag='recon_' + split, global_step=iterations, img_tensor=imgs_recon)
    train_writer.add_images(tag='nv_' + split, global_step=iterations, img_tensor=imgs_nv)
    train_writer.add_images(tag='nv2recon_' + split, global_step=iterations, img_tensor=imgs_nv2recon)
    train_writer.flush()
