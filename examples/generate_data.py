...
def get_display_loader(dataset, height, width, batch_size, workers, testset=None, mesh_dir=None):
	...
	test_loader = DataLoader(
		AllMeshPreprocessor(testset,root=dataset.images_dir, transform=test_transformer, mesh_dir=mesh_dir,mesh_transform=test_transformer),
		batch_size=batch_size, num_workers=workers,
		shuffle=False, pin_memory=True)
...
def main():
	...
	if args.dataset_target == 'market1501':
		checkpoint_path = '/data/stars/user/hchen/Projects/multiview_reid/outputs/market_init_JVTC_nosource/checkpoints/stage3'
		...
		output_path = 'outputs/market_init_JVTC_nosource/images/stage3/test'
